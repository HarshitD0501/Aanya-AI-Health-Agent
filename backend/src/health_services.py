"""
Health Services & Facility Lookup Module for Day 5 - Voice for Bharat Challenge.
...
# Dataset freshness timestamp as required by Day 5 guidelines
DATA_SOURCE_ATTRIBUTION = "OpenStreetMap Live Directory & National Health Registry (Updated 2026)"
...
def fetch_live_openstreetmap_hospitals(location: str, timeout: float = 3.5) -> list[dict[str, Any]]:
    ...
    for q in queries_to_try:
        try:
            ...
            with urllib.request.urlopen(req, timeout=timeout) as response:
                if response.status == 200:
                    ...
                    for item in data:
                        ...
                        name = item.get("name") or (display_name.split(",")[0] if display_name else f"Hospital in {location.title()}")
            ...
            logger.warning(f"Live OpenStreetMap API lookup failed or timed out for '{q}': {e}")
    return []
...
def search_health_facilities(
    ...
    if simulated_failure:
        logger.error("Simulated network timeout connecting to National Health Registry API.")
        return {
            "status": "error",
    ...
    if facility_type and matches:
        filtered = [
            m for m in matches
            if facility_type.lower() in m["facility_type"].lower() or facility_type.lower() in m["name"].lower()
        ]
        if filtered:
            matches = filtered
